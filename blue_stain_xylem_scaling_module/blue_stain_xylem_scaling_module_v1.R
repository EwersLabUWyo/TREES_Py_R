...
# NOTES: - The call to ggplot and plotting code at the end are just for evaluation purposes. 
#        - Non-linear least squares regression are currently used to determine empirical model
#          parameter estimates.  

#:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::#
#                                                                             #
# Model to downscale xylem conductance (transpiration) as a function of bark  #
# beetle attack and associated blue stain fungal infection.                   #
#                                                                             #
#:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::#
...
setwd("C:\\Users\\Dave\\Documents\\TREES_Py_R\\bb_fungi_module")
...
at <- sf_decline$"at"
xs <- sf_decline$"sfd"   # 'xs' represents 'xylem scalar'
...
xylem_scalar <- function(temp_obs,gr_obs,at,xs){
  ...
  sim_bs_bm <- numeric(length(at))
  for (i in 2:length(sim_bs_bm)){
    sim_bs_bm[i] <- sim_bs_bm[i-1] + (a * exp(-0.5*((at[i]-b)/c)^2))
  }
  ...
  xs.fit <- nls(xs ~ 1/(1+a2*exp(b2*sim_bs_bm)), start = list(a2 = 0.04, b2 = 0.0006))
...
CP_xs_sim <- xylem_scalar(temp_obs,gr_obs,at,xs)
CP_xs_obs <- xs
